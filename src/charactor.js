...
class Charactor {
   ...
   * @property {import('./sprite').FramesMap} CharactorMeta.framesMap
  ...
  constructor({ framesMap, position, ctx }) {
    /**
     * The actual rendered times, for controlling actions interval.
     *
     * @protected
     */
  ...
  /**
   * Set up sprite frames according to current action.
   *
   * @private
   */
  setCurrentFrames() {
   ...
   * Action setter.
   *
   * Set the given action if the charactor has no other actions. `idle` will be interrupted and `attack` will be prioritized.
   *
   * Returns whether the action has been successfully set.
   *
   * @protected
   *
   ...
   *
   * Decide by whether exist true value in `directions`.
   *
   * @private
   */
  willMove() {
   ...
   *
   * Increase `actualFramesPast` by 1.
   *
   * @protected
   */
  render() {
...
export class Player extends Charactor {
   ...
   * Attack action.
   *
   * Return if the charactor has already attacted before `stop`
